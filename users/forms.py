...
class EntryPointAwareUserActionForm(forms.Form):
    ...
    def get_groups_queryset(self, user):
        ...
        if user.groups.count() == 0:
            ...
            entry_points_ids = user.groups.all().values_list("page_permissions__page_id", flat=True).distinct()
            return Group.objects.filter(page_permissions__page_id__in=entry_points_ids).distinct()
