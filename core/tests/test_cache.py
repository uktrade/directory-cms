...
@pytest.mark.parametrize('page_id,language_code,expected', (
    (
        1,
        None,
        'serialised-page[1]?lang=None'
    ),
    (
        2,
        'en-gb',
        'serialised-page[2]?lang=en-gb'
    ),
    (
        3,
        'fr',
        'serialised-page[3]?lang=fr'
    ),
))
def test_page_cache_build_keys(page_id, language_code, expected):
...
def test_subscriber_delete(mock_delete):
    ...
    instance = mock.Mock(
        id=1, translated_languages=['en-gb']
    )
